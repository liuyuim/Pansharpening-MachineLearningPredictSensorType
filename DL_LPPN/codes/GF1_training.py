...
# cd/d C:\Users\TQW\Shiyan\FusionEvaluateExperiment\MethodDL_LPPN\codes
# python GF1_training.py
...
from GF1_model import GaussianPyramid, LPPN
...
image_size = 64
model_directory = 'D:/LiuYu/shiyan/GF1_LPPN/SaveModel'  # directory to save trained model to.
# training data in .mat format
train_data_name = 'D:/LiuYu/shiyan/GF1_Data/Train/N_H_W_C.mat'
# validation data in .mat format
test_data_name = 'D:/LiuYu/shiyan/GF1_Data/Validation/N_H_W_C.mat'
...
if __name__ == '__main__':
    # tf.reset_default_graph函数用于清除默认图形堆栈并重置全局默认图形。 无论执行多少次生成的张量始终不变。
    tf.compat.v1.reset_default_graph()
    ...
    with tf.compat.v1.Session() as sess:
        ...
        print('pan1 shape:', pan1.shape)
        print('ms_lr1 shape:', ms_lr1.shape)
        print('lms1 shape:', lms1.shape)
        ...
        gt1 = np.array(gt1, dtype=np.float32) / \
            1023.  # normalization, WorldView L = 11
        pan1 = np.array(pan1, dtype=np.float32) / 1023.
        ms_lr1 = np.array(ms_lr1, dtype=np.float32) / 1023.
        ...
        print('pan2 shape:', pan2.shape)
        print('ms_lr2 shape:', ms_lr2.shape)
        print('lms2 shape:', lms2.shape) 

        gt2 = np.array(gt2, dtype=np.float32) / \
            1023.  # normalization, WorldView L = 11
        pan2 = np.array(pan2, dtype=np.float32) / 1023.
        ms_lr2 = np.array(ms_lr2, dtype=np.float32) / 1023.
        lms2 = np.array(lms2, dtype=np.float32) / 1023.
        N2 = gt2.shape[0]
        ...
        for i in range(iterations):
            bs = train_batch_size
            batch_index = np.random.randint(0, N, size=bs) # 返回一个随机整型数，范围从低（包括）到高（不包括），输出随机数的尺寸
            ...
            # print('pan_batch shape:', pan_batch.shape)
            # print('ms_lr_batch shape:', ms_lr_batch.shape)
            # print('train_lms shape:', train_lms.shape) 
            ...
            # print('test_gt_batch shape:', test_gt_batch.shape)
            # print('test_pan_batch shape:', test_pan_batch.shape)
            # print('test_ms_lr_batch shape:', test_ms_lr_batch.shape)
            # print('test_lms_batch shape:', test_lms_batch.shape) 
