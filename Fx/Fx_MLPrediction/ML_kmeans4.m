...
function [] = ML_kmeans4(EvaluationDirList,TrainProportion,columnSample)
...
for  i= 1:3
    ...
    plot3(features(plotind,index_id(1)),features(plotind,index_id(2)),features(plotind,index_id(3)),markerArray(i),'MarkerSize',markerSize,'MarkerFaceColor',colorArray(i),'MarkerEdgeColor','none')
end

% 在XY平面上添加投影点
for i = 1:length(features)
    plot3(features(i,index_id(1)), features(i,index_id(2)), 0, ...
          markerArray(speciesNum(i)), 'MarkerSize', markerSize, 'MarkerEdgeColor', colorArray(speciesNum(i)), 'MarkerFaceColor','none');
end
...
h = zeros(1, 6);
for i = 1:3
    h(i) = plot3(nan, nan, nan, markerArray(i), 'MarkerSize', markerSize, 'MarkerEdgeColor', 'none', 'MarkerFaceColor', colorArray(i));
    h(i+3) = plot3(nan, nan, nan, markerArray(i), 'MarkerSize', markerSize, 'MarkerEdgeColor', colorArray(i), 'MarkerFaceColor', 'none');
end
...
grid on
view([60,24])
xlabel('GF-1 Pansharpening Model')
ylabel('QB Pansharpening Model')
zlabel('WV4 Pansharpening Model')
title('QNRs Obtained From Pansharpened Images ')
set(gca,'FontSize',12)
set(gca,'FontWeight','Bold')

% 添加图例
legend(h,'GF1 Image', 'QB Image', 'WV4 Image', 'GF1 Projection', 'QB Projection', 'WV4 Projection')
