...
def get_batch(train_data, bs):
    ...
    gt    = np.array(gt, dtype=np.float32) / 1023. #2047  ### normalization, WorldView L = 11
    pan   = np.array(pan, dtype=np.float32) /1023. #2047
    ms_lr = np.array(ms_lr, dtype=np.float32) / 1023. #2047
    lms   = np.array(lms, dtype=np.float32) /1023. #2047
...
# if __name__ =='__main__':
    ...
    # run_main(model_directory,train_data_name,test_data_name)
