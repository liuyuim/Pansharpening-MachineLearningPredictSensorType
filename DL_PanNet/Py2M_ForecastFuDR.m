...
saveDir = '..\..\Tmp\DataDLForecast\PannetEvaluate\';%设置对应保存路径
...
%             saveName = fullfile(saveDir,ErjiDir_list(ErjiDir_i).name,[num2str(i),'.mat']);
            ...
            save(saveName, 'TestOutputPath', 'MatrixResult_DR','MatrixResult_Fu');
        ...
        save(saveName, 'TestOutputPath','MatrixResults_Fu', 'MatrixResults_DR','Mean_Fu','Mean_DR','median_Fu','median_DR','max_Fu','max_DR','min_Fu','min_DR','ZX95_Fu','ZX95_DR');
...
%         Xlsx_zhi(10,:) =  ZX95_DR;
        ...
        switch(ErjiDir_list(i_ErjiDir).name)
            ...
            case 'HypothesisInGF1model'
               ...
               && max_Fu(:,2)<0.4048 && min_Fu(:,2)>0.1308 ... % D_S
               && max_Fu(:,3)<0.8575 && min_Fu(:,3)>0.5802 ... % QNRI
               && max_Fu(:,4)<1.3546 && min_Fu(:,4)>0.9757 ... % SAM
               && max_Fu(:,5)<0.6750 && min_Fu(:,5)>0.5709  % SCC
                ScoreGF1_Max_FuMin_Fu = 1;                    
                end
                % 检查 Mean_Fu 相似度
            ...
            case 'HypothesisInGF2model'
               ...
               && max_Fu(:,2)<0.9059 && min_Fu(:,2)>0.7439 ... % D_S
               && max_Fu(:,3)<0.2319 && min_Fu(:,3)>0.0864 ... % QNRI
               && max_Fu(:,4)<1.8317 && min_Fu(:,4)>0.8198 ... % SAM
               && max_Fu(:,5)<0.6812 && min_Fu(:,5)>0.5411  % SCC
                ScoreGF2_Max_FuMin_Fu = 1;                    
                end
                % 检查 Mean_Fu 相似度
            ...
            case 'HypothesisInQBmodel'
               ...
               && max_Fu(:,2)<0.8695 && min_Fu(:,2)>0.5065 ... % D_S
               && max_Fu(:,3)<0.4136 && min_Fu(:,3)>0.1247 ... % QNRI
               && max_Fu(:,4)<1.7640 && min_Fu(:,4)>1.3573 ... % SAM
               && max_Fu(:,5)<0.8495 && min_Fu(:,5)>0.7781  % SCC
                ScoreQB_Max_FuMin_Fu = 1;                    
                end
                % 检查 Mean_Fu 相似度
            ...
            case 'HypothesisInWV2model'
               ...
               && max_Fu(:,2)<0.6428 && min_Fu(:,2)>0.3491 ... % D_S
               && max_Fu(:,3)<0.5785 && min_Fu(:,3)>0.3515 ... % QNRI
               && max_Fu(:,4)<2.7797 && min_Fu(:,4)>1.7232 ... % SAM
               && max_Fu(:,5)<0.7538 && min_Fu(:,5)>0.5436  % SCC
                ScoreWV2_Max_FuMin_Fu = 1;                    
                end
                % 检查 Mean_Fu 相似度
            ...
            case 'HypothesisInWV3model'
               ...
               && max_Fu(:,2)<0.8170 && min_Fu(:,2)>0.4388 ... % D_S
               && max_Fu(:,3)<0.5488 && min_Fu(:,3)>0.1789 ... % QNRI
               && max_Fu(:,4)<2.7441 && min_Fu(:,4)>1.0446 ... % SAM
               && max_Fu(:,5)<0.8389 && min_Fu(:,5)>0.6111  % SCC                
                ScoreWV3_Max_FuMin_Fu = 1;                    
                end
                % 检查 Mean_Fu 相似度
...
%         save(SaveName, 'MatrixResults_Fu', 'MatrixResults_DR');
    ...
    formatSpec = "目录%s已完成评价，请到%s查看！预测遥感影像为【%s传感器】\n ";
