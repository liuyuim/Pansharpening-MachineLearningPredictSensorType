...
def run_TestHypothesis(Datapath,saveDir,model_directory_list,HypothesisDir_list):
    ...
    if(len(model_directory_list) != len(HypothesisDir_list)):
        print("检测到 传感器种类数 和 对应的model数 不匹配 程序退出。\n 请检查run_TestHypothesis()函数中 sensor_list 和 model_directory_list两个列表 ")
    ...
    for j in range(len(model_directory_list)): 
        ...
        for MatName in MatName_list:
            ...
            if fnmatch.fnmatch(HypothesisDir, '*GF1*')|fnmatch.fnmatch(HypothesisDir, '*GF2*'):
                ...
                print("检测HypothesisDir是IK/JL/QB/WV...使用的融合代码是 Runtest4b2047 ... ")
...
if __name__=='__main__': # 按多种模型生成假设结果，并保存到对应的假设结果文件夹中  
    ...
    model_directory_list = [
                            'O:/HC550WDC16TO/Shiyan/20231201/GF1_PanNet/SaveModel/',
                            # 'O:/HC550WDC16TO/Shiyan/20231201/GF2_PanNet/SaveModel/',
                            'O:/HC550WDC16TO/Shiyan/20231201/IK_PanNet/SaveModel/',
                            # 'O:/HC550WDC16TO/Shiyan/20231201/JL1_PanNet/SaveModel/',
                            'O:/HC550WDC16TO/Shiyan/20231201/WV2_PanNet/SaveModel/', 
                            'O:/HC550WDC16TO/Shiyan/20231201/WV3_PanNet/SaveModel/',
                            'O:/HC550WDC16TO/Shiyan/20231201/WV4_PanNet/SaveModel/',
                            'O:/HC550WDC16TO/Shiyan/20231201/QB_PanNet/SaveModel/'
                            ]
    HypothesisDir_list = [
    ...
    # 数据集SensorNames = ['GF1', 'GF2', 'IK', 'JL1', 'QB', 'WV2', 'WV3', 'WV4']
    SensorNames = ['GF1','IK','QB','WV2','WV3','WV4'] # 数据集SensorNames = ['GF1','IK','QB','WV2','WV3','WV4'] ['GF1','GF2','JL1','QB','WV2','WV3']
    # SensorNames = ['JL1'] # SensorNames = ['GF1','GF2','JL1','QB','WV2','WV3']
    for sensor_name in SensorNames:

        Sensor_Data = sensor_name + '_Data'
        Sensor_Net = sensor_name + '_PanNet'
        Base_dir = r'O:/HC550WDC16TO/Shiyan/20231201'
        Sizes = ["1024","512","256","128","64","32"] # Sizes = ["1024","512","256","128","64","32"]
        for Size in Sizes:
            
            Test_Fu = 'Test_Fu' + Size        # Test_Fu Test_DR   
            HypothesisOutput_Fu = 'HypothesisOutput_Fu' + Size # HypothesisOutput_Fu HypothesisOutput_DR
    ...
    # Datapath = 'F:/AFusionGroup/Shiyan/shiyan20231201/QB_Data/Test_Fu1024'
    # saveDir = 'F:/AFusionGroup/Shiyan/shiyan20231201/QB_PanNet/HypothesisOutput_Fu1024'
    # run_TestHypothesis(Datapath,saveDir,model_directory_list,HypothesisDir_list)