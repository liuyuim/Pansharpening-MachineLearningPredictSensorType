...
def load_set(file_path):
    ...
    lms = torch.from_numpy(
        data['lms'] / 1023.0).permute(2, 0, 1)  # CxHxW = 8x256x256
    ms = torch.from_numpy((data['ms'] / 1023.0)
                          ).permute(2, 0, 1)  # CxHxW= 8x64x64
    pan = torch.from_numpy((data['pan'] / 1023.0))   # HxW = 256x256
...
# python Runtrain4b1023.py

if __name__ == "__main__":

    
    # train_set = Dataset_Pro('F:/AFusionGroup/Shiyan/shiyan20230902/GF1_Data/Train/N_C_H_W.mat')  
    # training_data_loader = DataLoader(dataset=train_set, num_workers=0, batch_size=batch_size, shuffle=True,
    #                                   pin_memory=True, drop_last=True)  # put training data to DataLoader for batches
    # validate_set = Dataset_Pro('F:/AFusionGroup/Shiyan/shiyan20230902/GF1_Data/Validation/N_C_H_W.mat')
    # validate_data_loader = DataLoader(dataset=validate_set, num_workers=0, batch_size=batch_size, shuffle=True,
    #                                   pin_memory=True, drop_last=True)  # put training data to DataLoader for batches
    # model_out_path = 'F:/AFusionGroup/Shiyan/shiyan20230902/GF1_WSDFNet/SaveModel'
    # if not os.path.exists(model_out_path):
    #     os.makedirs(model_out_path)
    # train(training_data_loader, validate_data_loader, model_out_path)

    train_set = Dataset_Pro('F:/AFusionGroup/Shiyan/shiyan20230902/GF2_Data/Train/N_C_H_W.mat')  
    training_data_loader = DataLoader(dataset=train_set, num_workers=0, batch_size=batch_size, shuffle=True,
                                      pin_memory=True, drop_last=True)  # put training data to DataLoader for batches
    validate_set = Dataset_Pro('F:/AFusionGroup/Shiyan/shiyan20230902/GF2_Data/Validation/N_C_H_W.mat')
    validate_data_loader = DataLoader(dataset=validate_set, num_workers=0, batch_size=batch_size, shuffle=True,
                                      pin_memory=True, drop_last=True)  # put training data to DataLoader for batches
    model_out_path = 'F:/AFusionGroup/Shiyan/shiyan20230902/GF2_WSDFNet/SaveModel'
    if not os.path.exists(model_out_path):
        os.makedirs(model_out_path)
    train(training_data_loader, validate_data_loader, model_out_path)