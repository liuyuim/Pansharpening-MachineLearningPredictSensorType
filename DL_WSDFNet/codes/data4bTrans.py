...
class Dataset_Pro(data.Dataset):
    def __init__(self, file_path):
        ...
        gt1 = np.array(gt1.transpose(3, 0, 2, 1), dtype=np.float32) / 2047
        ...
        lms1 = np.array(lms1.transpose(3, 0, 2, 1), dtype=np.float32) / 2047
        self.lms = torch.from_numpy(lms1)
        print('lms:',self.lms.shape)

        ms1 = data["ms"][...]  # NxCxHxW
        # ms1 = np.array(ms1.transpose(0, 2, 3, 1),
        ms1 = np.array(ms1.transpose(3, 2, 1, 0),
        ...
        pan1 = np.array(pan1.transpose(3, 2, 1, 0),
        ...
        pan1 = np.array(pan1.transpose(3, 0, 2, 1), dtype=np.float32) / 2047  # Nx1xHxW
